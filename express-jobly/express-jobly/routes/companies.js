...
// Using the multi filter which has three filters 
router.get("/", async function (req, res, next) {
  try {
    let companies = await Company.multi_filter(
      req.query.name,
    ...
    if (req.query.minEmployees && req.query.maxEmployees) {
      const min = parseInt(req.query.minEmployees);
      const max = parseInt(req.query.maxEmployees);
      if (min > max) {
        throw new ExpressError("Minimum has to be less than maximum", 400);
      }
    }
