...
describe("GET /companies", function () {
  ...
  test("minEmployee query", async function () {
  ...
  test("maxEmployee query", async function () {
  ...
  test("min>max", async function () {
    const resp = await request(app).get(
      "/companies?maxEmployees=2&minEmployees=3"
    );
    expect(resp.statusCode).toEqual(400);
    expect(resp.body).toEqual({
      error: { message: "Minimum has to be less than maximum", status: 400 },
    });
  });

  test("minEmployee+maxEmployee query", async function () {
    const resp = await request(app).get(
      "/companies?maxEmployees=2&minEmployees=2"
    );
